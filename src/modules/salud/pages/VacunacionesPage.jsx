...
const VacunacionesPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setModalOpen(false);
      setFormData({ lote: '', fecha: new Date().toISOString().split('T')[0], tipo_vacuna: '', cantidad_aves: '', metodo_aplicacion: '', observaciones: '' });
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Plan de vacunación de lotes</p>
        </div>
        <Button icon={Plus} onClick={() => setModalOpen(true)}>Nueva Vacunación</Button>
      </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Nueva Vacunación" size="sm">
        <form onSubmit={handleSubmit} className="space-y-4">
          <Select label="Lote" value={formData.lote} onChange={(e) => setFormData({...formData, lote: e.target.value})} options={lotes.map(l => ({ value: l.id, label: l.nombre }))} required />
          <Input label="Fecha" type="date" value={formData.fecha} onChange={(e) => setFormData({...formData, fecha: e.target.value})} required />
          <Input label="Tipo de Vacuna" value={formData.tipo_vacuna} onChange={(e) => setFormData({...formData, tipo_vacuna: e.target.value})} required />
          <Input label="Cantidad de Aves" type="number" value={formData.cantidad_aves} onChange={(e) => setFormData({...formData, cantidad_aves: e.target.value})} required />
          <Input label="Método de Aplicación" value={formData.metodo_aplicacion} onChange={(e) => setFormData({...formData, metodo_aplicacion: e.target.value})} required />
          <Input label="Observaciones" value={formData.observaciones} onChange={(e) => setFormData({...formData, observaciones: e.target.value})} multiline />
          <div className="flex gap-3">
            <Button variant="secondary" onClick={() => setModalOpen(false)} className="flex-1">Cancelar</Button>
            <Button type="submit" loading={createVacunacion.isPending} className="flex-1">Registrar</Button>
          </div>
        </form>
      </Modal>
