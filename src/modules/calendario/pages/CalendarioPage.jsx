...
const CalendarioPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setModalOpen(false);
      setFormData({ tipo: '', titulo: '', descripcion: '', fecha_inicio: new Date().toISOString().slice(0, 16), fecha_fin: '', todo_el_dia: false });
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Nuevo Evento" size="md">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Input label="Fecha y Hora de Inicio" type="datetime-local" value={formData.fecha_inicio} onChange={(e) => setFormData({...formData, fecha_inicio: e.target.value})} required />
          <Input label="Fecha y Hora de Fin (opcional)" type="datetime-local" value={formData.fecha_fin} onChange={(e) => setFormData({...formData, fecha_fin: e.target.value})} />
